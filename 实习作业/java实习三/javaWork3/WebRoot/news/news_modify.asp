	          ...
	          <tr>
	            <td valign="top"><form ACTION="<%=MM_editAction%>" METHOD="POST" name="form1">
	              <table width="100%" border="0" cellspacing="0" cellpadding="0">
	                <tr>
	                  <td height="41" colspan="2" align="center" bgcolor="#607F42" class="text1" >修 改 新 闻</td>
	                  </tr>
	                <tr>
	                ...
	                <tr>
	                  ...
	                  <td height="39"><span id="spryselect1">
	                    <select name="news_type" id="news_type">
	                      <option value="热点" <%If (Not isNull((news.Fields.Item("news_type").Value))) Then If ("热点" = CStr((news.Fields.Item("news_type").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%>>热点</option>
	                      <option value="公告" <%If (Not isNull((news.Fields.Item("news_type").Value))) Then If ("公告" = CStr((news.Fields.Item("news_type").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%>>公告</option>
	                      <option value="新闻" <%If (Not isNull((news.Fields.Item("news_type").Value))) Then If ("新闻" = CStr((news.Fields.Item("news_type").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%>>新闻</option>
	                      <option value="活动" <%If (Not isNull((news.Fields.Item("news_type").Value))) Then If ("活动" = CStr((news.Fields.Item("news_type").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%>>活动</option>
	                      <option value="社论" <%If (Not isNull((news.Fields.Item("news_type").Value))) Then If ("社论" = CStr((news.Fields.Item("news_type").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%>>社论</option>
	                      <%
While (NOT news.EOF)
%>
...
%>
	                ...
	                <tr>
	                  <td height="49" colspan="2" align="center"><input name="news_time" type="hidden" id="news_time" value="<%=now()%>">
	                    <input type="submit" name="button" id="button" value="提交修改">
	                    &nbsp; &nbsp;&nbsp; <input type="reset" name="button2" id="button2" value="重新修改"></td>
	                  </tr>
	                </table>
