...
<%	str = request.getParameter("submit");
	if(str!=null&&str.equals("确  定")){
		...
		System.out.println(ifStr+": "+thenStr);		
		sql.addTask(userId, tskName, Integer.parseInt(ifType), ifStr, Integer.parseInt(thenType), thenStr,	"修改成功!");
		response.sendRedirect("UserTaskDetail.jsp?tskName="+tskName);
		return;
	}
%>
...
<h2>新建您的任务</h2>
<p>&nbsp;</p>
<p class="Label3"> <label>任务名： <%=tskName %></label></p>
...
<td style="visibility:hidden;" id="timeLable">
  ...
  <input name="mailPw" type="text" id="mailPw" />
  </p>
  </td>
  </tr>
</table>
...
<input type="hidden"" name="tskName" id="tskName" value=<%=tskName %> />
...
<p align="center">
   		<input type="submit" name="submit" id="submit" value="确  定" onclick="return clickCreate();">
        &nbsp; &nbsp;&nbsp; &nbsp; 
...
<p>&nbsp;</p>
    ...
    <table cellspacing="10" class="fix">
    	<td><a href="UserTask.jsp" target="_self">查看任务</a></td>
    </table>
