...
<%	str = request.getParameter("start");
	if(str!=null&&str.equals("true")){
		...
		if(rs.getString(7).trim().equals(Task.READY)){
			...
			sql.sendMail("yz", vipId, "任务变更", "管理员："+userId+"开始了您的任务："+tskName);
		}
		else if(rs.getString(7).trim().equals(Task.SUSPEND)){
			sql.updateTaskState(vipId, tskName, Task.CONTINUE);
			sql.sendMail("yz", vipId, "任务变更", "管理员："+userId+"开始了您的任务："+tskName);
		}
		response.sendRedirect("TaskManage.jsp?vipId="+vipId);
		return;
	}
	...
	if(str!=null&&str.equals("true")){
		...
		if(rs.getString(7).trim().equals(Task.RUNNING) || rs.getString(7).trim().equals(Task.CONTINUE)){
			...
			sql.sendMail("yz", vipId, "任务变更", "管理员暂停了您的任务："+tskName);
		}
		response.sendRedirect("TaskManage.jsp?vipId="+vipId);
		return;
	}
	...
	if(str!=null&&str.equals("true")){
		...
		if(rs.getString(7).trim().equals(Task.RUNNING) || rs.getString(7).trim().equals(Task.SUSPEND)
			...
			sql.sendMail("yz", vipId, "任务变更", "管理员："+userId+"结束了您的任务："+tskName);
		}
		response.sendRedirect("TaskManage.jsp?vipId="+vipId);
		return;
	}
	...
	if(str!=null&&str.equals("true")){
		...
		if(rs.getString(7).trim().equals(Task.READY) || rs.getString(7).trim().equals(Task.END)){
			...
			sql.sendMail("yz", vipId, "任务变更", "管理员："+userId+"删除了您的任务："+tskName);
		}
		response.sendRedirect("TaskManage.jsp?vipId="+vipId);
		return;
	}
...
<div class="header">
  ...
  <div class="sidebar1">
     ...
     <a href="ManagerMain.jsp" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('首页公告','','image/buttoned1.png',1)"><img src="image/button1.png" alt="首页公告" width="100%" height="100" id="首页公告" /></a>
       <a href="VIPManage.jsp" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('会员管理','','image/buttoned3.png',1)"><img src="image/button3.png" alt="会员管理" width="100%" height="100" id="会员管理" /></a>
      <a href="TaskManage.jsp" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('任务管理','','image/buttoned4.png',1)"><img src="image/buttoned4.png" alt="任务管理" width="100%" height="100" id="任务管理" /></a>
    </ul>
...
<%	while(rs.next()){
		String vipId = rs.getString(1).trim();
		...
		+"<td><a href=TaskManage.jsp?start=true&tskName="+tskName+"&vipId="+vipId+">开  始</a>   "
		+"<a href=TaskManage.jsp?suspend=true&tskName="+tskName+"&vipId="+vipId+">暂 停</a>   "
		+"<a href=TaskManage.jsp?end=true&tskName="+tskName+"&vipId="+vipId+">结 束</a>   "
		+"<a href=TaskManage.jsp?delete=true&tskName="+tskName+"&vipId="+vipId+">删 除</a></td></tr>");
	} 
%>
    </table>
