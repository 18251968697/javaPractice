...
<%	str = request.getParameter("delete");
	if(str != null && str.equals("true")){
		...
		sql.deleteUser(vipId);
		response.sendRedirect("VIPManage.jsp");
		return;
	}
%>
...
<%	rs = sql.queryAllVipInfo();
	while(rs.next()){
		...
		out.print("<tr align=center><td><a href=VIPInfoDisplay.jsp?vipId="+vipId+"  />"+vipId+"</td><td>"
				+level+"</td><td>"+score+"</td><td>"+discount+"</td><td>"+balance
				+"</td><td><a href=VIPTransDisplay.jsp?vipId="+vipId+">查 看</td>"
				+"<td><a href=VIPTaskDisplay.jsp?vipId="+vipId+">查 看</td>"
				+"<td><a href=VIPManage.jsp?delete=true&vipId="+vipId+" />删 除</td></tr>");
	} 
%>
    </table>
